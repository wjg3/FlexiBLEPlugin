...
#include "../include/FlexiBLEKernels.h"
#include <cmath>
#include <map>
#include <sstream>
#include <algorithm>
...
double SlicedNonbondedForceImpl::evalIntegral(double r, double rs, double rc, double sigma)
{
    ...
    double r6 = r * r5;
    double r9 = r3 * r6;
    return sig6 * A3 * ((sig6 * (+rs3 * 28 * (6 * rs2 * A2 + 15 * rs * A + 10) - r * rs2 * 945 * (rs2 * A2 + 2 * rs * A + 1) + r2 * rs * 1080 * (2 * rs2 * A2 + 3 * rs * A + 1) - r3 * 420 * (6 * rs2 * A2 + 6 * rs * A + 1) + r4 * 756 * (2 * rs * A2 + A) - r5 * 378 * A2) - r6 * (+rs3 * 84 * (6 * rs2 * A2 + 15 * rs * A + 10) - r * rs2 * 3780 * (rs2 * A2 + 2 * rs * A + 1) + r2 * rs * 7560 * (2 * rs2 * A2 + 3 * rs * A + 1))) / (252 * r9) - log(r) * 10 * (6 * rs2 * A2 + 6 * rs * A + 1) + r * 15 * (2 * rs * A2 + A) - r2 * 3 * A2);
}
